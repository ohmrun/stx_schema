...
class BaseType extends EqCls<TBaseType>{
  ...
  public function comply(thiz:TBaseType,that:TBaseType){
    var eq = new stx.assert.schema.type.eq.Identity().comply(thiz.identity,that.identity);
    if(eq.is_equal()){
      eq = new stx.assert.schema.type.eq.Validations().comply(thiz.validation,that.validation);
    }
    if(eq.is_equal()){

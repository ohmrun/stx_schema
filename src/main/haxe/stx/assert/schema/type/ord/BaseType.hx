...
class BaseType extends OrdCls<TBaseType>{
  ...
  public function comply(thiz:TBaseType,that:TBaseType){
    var ord = new stx.assert.schema.type.ord.Identity().comply(thiz.identity,that.identity);
    if(ord.is_not_less_than()){
      ord = new stx.assert.schema.type.ord.Validations().comply(thiz.validation,that.validation);
    }
    if(ord.is_not_less_than()){

...
@:forward abstract ProcureAttribute(ProcureAttributeDef) from ProcureAttributeDef to ProcureAttributeDef{
  ...
  @:noUsing static public function make(name,type,relation,inverse,meta,?validation){
    return lift({
      name        : name,
      type        : type,
      relation    : relation,
  ...
  public function with_type(type:SchemaRef){
    return make(this.name,type,this.relation,this.inverse,this.meta,this.validation);
  }
